...
export class BigDecimal {
  ...
  static fromMetric(
    { decimals, exact }: { decimals: number; exact: string } = {
      decimals: 18,
      exact: '0',
    },
  ): BigDecimal {
  ...
  scale(decimals: number = DEFAULT_DECIMALS): BigDecimal {
    ...
    if (this.exact.eq(0)) {
      ...
      scaled =
        this.decimals > decimals
          ? this.exact.mul(otherScale).div(thisScale)
          : this.exact.mul(otherScale.div(thisScale));
    }
  ...
  add(other: BigDecimal | BigNumber): BigDecimal {
    return this.transform(
      this.exact.add(other instanceof BigNumber ? other : other.exact),
    );
  }

  sub(other: BigDecimal | BigNumber): BigDecimal {
    return this.transform(
      this.exact.sub(other instanceof BigNumber ? other : other.exact),
    );
  }
